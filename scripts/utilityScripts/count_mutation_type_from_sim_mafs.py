...
def execute_grep_count_subprocess(cmd, countStr):
	try:
		data = subprocess.check_output(cmd)
		count = data.count(countStr)
...
simulatedMafFileDir = '/ifs/work/taylorlab/friedman/myAdjustedDataFiles/simulatedMafs/geneMutMafs'

#geneChromosomeDict = analysis_utils.map_impact_genes_to_chromosome(impactRegionsFile = pathPrefix + '/ifs/work/taylorlab/friedman/myAdjustedDataFiles/genelist.with_aa.interval_list')
mafsInDir = os.listdir(simulatedMafFileDir)
listOfDicts = []
...
tNonsenseVar = 0 
tSpliceSiteVar = 0
tTranslationStartSiteVar = 0

for gene in list(impactGenes):
	cntr += 1
	d = {}
	print 'analyzing gene ', gene, ' which is the number ', cntr, ' gene'

	mafName = gene + '_all_possible_snps_v2.maf'
	mafPath = os.path.join(simulatedMafFileDir, mafName)
	#we run various processes to count things

	#FIRST the number of lines which is the number of possible mutations
	commandNLines =['wc', '-l', mafPath]
	nLines = subprocess.check_output(commandNLines).split(' ')[0]

	if nLines < 10:
		d['Hugo_Symbol'] = gene #have an NA entry for any genes we haven't looked at in detail yet
		listOfDicts.append(d)

	else:
		commandNMissenseVar = ['grep', '-o', 'Missense_Mutation', mafPath]
		commandNNonsenseVar = ['grep',  '-o', 'Nonsense_Mutation', mafPath]
		commandNSpliceSiteVar = ['grep', '-o', 'Splice_Site', mafPath]
		commandNTranslationStartSiteVar = ['grep',  '-o', 'Translation_Start_Site', mafPath]

		nMissense = execute_grep_count_subprocess(commandNMissenseVar, '\n')
		nNonsenseVar  = execute_grep_count_subprocess(commandNNonsenseVar, '\n')
		nSpliceSiteVar = execute_grep_count_subprocess(commandNSpliceSiteVar, '\n')
		nTranslationStartSiteVar = execute_grep_count_subprocess(commandNTranslationStartSiteVar, '\n')

		tMissense += nMissense
		tNonsenseVar += nNonsenseVar
		tSpliceSiteVar += nSpliceSiteVar
		tTranslationStartSiteVar += nTranslationStartSiteVar

print tMissense, tNonsenseVar, tSpliceSiteVar, tTranslationStartSiteVar
