...
#adds annotations to a maf with dep map scores for various genes
#requires a maf annotated with cancer type information
def annotate_maf_with_dep_map_cancer_type(maf, tcgaInfoPath='', impactInfoPath=''):

	#Note we only map relatively common aetiolgoies
	...
	print 'beginning annotation of exome maf; takes a long time'
	tcgaCancerTypeDict = get_gene_and_cohort_list_utils.get_tcga_cancer_type_info(tcgaInfoPath = tcgaInfoPath, cancerTypes = [])
	impactCancerTypeDict = get_gene_and_cohort_list_utils.get_impact_cancer_type_info(impactCancerTypeInfoPath = impactInfoPath)

	maf['Tumor_Sample_Barcode'] = maf['Tumor_Sample_Barcode'].apply(lambda x:
                        x[:12] if len(x) > 12
                       else x)

	print 'assigning cancer type info'
	maf['cancerType'] = maf['Tumor_Sample_Barcode'].apply(lambda x: 
    	tcgaCancerTypeDict[x] if x in tcgaCancerTypeDict
    	else impactCancerTypeDict[x] if x in impactCancerTypeDict
    	else None)


	print 'assiging dep map cancer type info'
	maf['cancerTypeDepMap'] = maf.apply(lambda row:
        impactToDepMapCancerTypeMapping[row['cancerType']]
            if row['cancerType'] in impactToDepMapCancerTypeMapping
        else tcgaToDepMapCancerTypeMapping[row['cancerType']]
            if  row['cancerType'] in tcgaToDepMapCancerTypeMapping
        else 'other', axis=1)
