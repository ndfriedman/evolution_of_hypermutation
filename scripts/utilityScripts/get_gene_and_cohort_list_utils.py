...
def set_up_dep_map_data(depMapPath = '~/Desktop/hypermutationProjectFinal/files/infoFiles/projectAchillesDepMapData.csv', cancerTypeLines = [], loadedDf = pd.DataFrame([{}])):
	...
	depMapData = None
	if loadedDf.shape[0] > 1:
		depMapData = loadedDf
	else:
		depMapData = pd.read_csv(depMapPath)

	if len(cancerTypeLines) > 0:
		depMapData = depMapData[depMapData['DepMap_ID'].isin(cancerTypeLines)]
...
def get_essential_genes(depMapPath = '~/Desktop/hypermutationProjectFinal/files/infoFiles/projectAchillesDepMapData.csv', mode='getEssentialGenes', cancerTypeLines = []):
	depMapDf = set_up_dep_map_data(depMapPath = depMapPath, cancerTypeLines = cancerTypeLines)
...
def get_cancer_neutral_genes(depMapPath = '~/Desktop/hypermutationProjectFinal/files/infoFiles/projectAchillesDepMapData.csv', cancerTypeLines = []):
...
def get_cancer_type_specific_essential_genes(cancerTypes, essentialThresh = -1):
	dictOfDepMapData = get_cancer_type_specific_dep_map_data(cancerTypes)
	d = {}
	for cancerType, cTypeDict in dictOfDepMapData.items():
		l = [key for key, value in cTypeDict.items() if value < essentialThresh]
		d[cancerType] = l
	return d

def get_expressed_genes_from_gtex(gtexDataPath = '', thresh=1):
	gtexData = pd.read_table(gtexDataPath, skiprows=[0,1])
	d = {}

	cancerTypesGtex = set(gtexData.columns.values) - set(['Name', 'Description'])
	print cancerTypesGtex
	for cancerType in cancerTypesGtex:
		expressedGenes = set(gtexData[gtexData[cancerType] > thresh]['Description'])
		d[cancerType] = expressedGenes
	return d
...
def get_exome_signature_cohorts(tcgaSigPath = '', exomeSigPath = ''):
	...
	tcgaSignatures['Tumor_Sample_Barcode'] = tcgaSignatures['Sample Name'].apply(lambda x: x[:15])

	return dict(zip(exomeRecaptureSignatures['Sample Name'], exomeRecaptureSignatures['dominantSignature'])), dict(zip(tcgaSignatures['Tumor_Sample_Barcode'], tcgaSignatures['dominantSignature']))
